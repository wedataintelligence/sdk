...
bool SockInfo::createAssociateEvent()
{
    if (handle == WSA_INVALID_EVENT)
    {
        ...
        //LOG_warn << "Created event " << fd << " " << handle;
    }
    if (handle == WSA_INVALID_EVENT)
    {
    ...
    if (associatedHandleEvents != events)
    {
        ...
        //LOG_warn << "Associated event " << fd << " " << handle << " " << events;
...
bool SockInfo::checkEvent(bool& read, bool& write)
{
    if (handle != WSA_INVALID_EVENT)
    {
        ...
        if (!write && (FD_WRITE & associatedHandleEvents))
        {
            ...
            //LOG_warn << "MANUAL CHECK WRITEABLE SOCKET on " << fd << " " << handle << " for " << associatedHandleEvents << "( from " << wne.lNetworkEvents << ")" << "writeResult " << writeResult << " writeError " << writeError;
        }

        if (read || write)
        {
            signalledWrite = signalledWrite || write;
            //if (signalledWrite) { LOG_warn << "SIGNALLEDWRITE SETFOR " << fd; }
            //LOG_warn << "NON-TRIGGERED Event on " << fd << " " << handle << " for " << associatedHandleEvents << " " << read << " " << write << "( from " << wne.lNetworkEvents << ")";
            return true;
        }
    }
    return false;
...
void SockInfo::closeEvent()
{
    if (handle != WSA_INVALID_EVENT)
    {
        //LOG_warn << "Closing " << fd << " " << handle << " for " << associatedHandleEvents;
...
CurlHttpIO::CurlHttpIO(Waiter* cw)
    : clientWaiter(cw)
{
...
//                                   
    ...
    curlThread.reset(new std::thread([this]() { thread_io();  }));
}
...
void CurlHttpIO::addevents(Waiter* w, int)
{
    ...
    WAIT_CLASS* waiter = (WAIT_CLASS*)w;
...
void CurlHttpIO::send_request(CurlHttpContext* httpctx)
{
    ...
    else
    {
        ...
        {
            LOG_warn << httpctx->req->logname << "Sending " << req->out->size() << ": " << *req->out;
        }
        else
        {
            LOG_warn << httpctx->req->logname << "Sending " << req->out->size() << ": " << req->out->substr(0, 5116) << " [...] " << req->out->substr(req->out->size() - 5116, string::npos);
        }
    }
    ...
    if ((curl = curl_easy_init()))
    {
        ...
        httpio->numconnections++;
        httpctx->curl = curl;

        asyncQueue.push([httpio, curl]() {
            curl_multi_add_handle(httpio->curlmulti, curl);
        });
   }
    else
    {
    ...
    if (req->httpiohandle)
    {
        asyncQueue.push([req, this]() {
            ...
            if (httpctx->curl)
            {
                ...
                curl_multi_remove_handle(curlmulti, httpctx->curl);
...
bool CurlHttpIO::thread_io()
{
    ...
    for (;;)
    {
        ...
        {
            std::unique_lock<std::mutex> g(asyncQueue.m);
            while (!asyncQueue.q.empty())
            {
                auto f = std::move(asyncQueue.q.front());
                if (!f) return false;
                asyncQueue.q.pop_front();
                g.unlock();
                f();
                g.lock();
            }
        ...
        curl_multi_perform(curlmulti, &runninghandles);

        multidoio(curlmulti);

        int numfds = 0; // todo: figure out how to pass in an event that we can trigger this function to wake up
        curl_multi_wait(curlmulti, nullptr, 0, 1, &numfds);
        //if (numfds == 0)
        //{
        //    Sleep(1);
        //}
            ...
            if (msg->msg == CURLMSG_DONE)
            {
                ...
                if (req->httpstatus)
                {
                    ...
                    else
                    {
                        ...
                        {
                            LOG_warn << req->logname << "Received " << req->in.size() << ": " << req->in.c_str();
                        }
                        else
                        {
                            LOG_warn << req->logname << "Received " << req->in.size() << ": " << req->in.substr(0, 5116).c_str() << " [...] " << req->in.substr(req->in.size() - 5116, string::npos).c_str();
                        }
                    }
                }
...
size_t CurlHttpIO::read_data(void* ptr, size_t size, size_t nmemb, void* source)
{
    ...
    //LOG_warn << "SENDING CURL DATA: " << req << " " << nread << " bytes from pos " << req->outpos;
...
void CurlHttpIOAsyncQueue::push(std::function<void()>&& f)
{
    std::lock_guard g(m);
